...
def _run_pipeline(
    name: str, stages: list, ctx: PipelineContext, close_db: bool = True
) -> PipelineContext:
    """创建并执行 Pipeline"""
    event_bus = EventBus()
...
def _load_config_via_pipeline(config_path: str):
    ...
    result_ctx = _run_pipeline(
        "_config_load", [ConfigLoadStage(), DBInitStage()], ctx, close_db=False
    )
...
def search(
    ...
    mode: str = typer.Option(
        "hybrid", "--mode", "-m", help="检索模式: hybrid/keyword/semantic"
    ),
    ...
    output: str = typer.Option(
        "console", "--output", "-o", help="输出格式: console/json/csv"
    ),
    config_path: str = typer.Option(
        "config.yaml", "--config", "-c", help="配置文件路径"
    ),
    ...
    if output == "json":
        output_data = [{"rank": i + 1, **r.__dict__} for i, r in enumerate(results)]
...
def status(
    ...
    try:
        ...
        if db and db.conn:
            ...
            if vaults:
                ...
                for v in vaults:
                    ...
                    typer.echo(
                        f"     {v['vault_name']}: {v['cnt']} files / {v_chunks} chunks"
                    )
        else:
        ...
        typer.echo(
            f"🔍 检索: alpha={config.retrieval.get('alpha', 0.7)}, beta={config.retrieval.get('beta', 0.3)}"
        )
        typer.echo(
            f"✂️ 分块: max_tokens={config.chunking.max_tokens}, overlap={config.chunking.overlap}"
        )
    finally:
        if db:
            db.close()
...
def maintenance(
    ...
    typer.secho(
        f"📊 软删除比例: files={stats.get('files_ratio', 0):.1f}%, chunks={stats.get('chunks_ratio', 0):.1f}%",
        fg=typer.colors.CYAN,
    )
    typer.secho(
        f"   数据库大小: {stats.get('file_size_mb', 0):.2f} MB", fg=typer.colors.CYAN
    )
...
def describe(
    pipeline_name: str = typer.Argument(
        "build", help="Pipeline 名称: build/scan/search/maintenance"
    ),
):
